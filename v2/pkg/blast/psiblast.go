...
func (p *PSIBlast) UnmarshalJSONObject(dec *gojay.Decoder, key string) error {
	if v, e := p.QueryConfig.UnmarshalJSONObject(dec, key); v || e != nil {
		return e
	}
	if v, e := p.WithIPGListContainer.UnmarshalJSONObject(dec, key); v || e != nil {
		return e
	}
	if v, e := p.WithBestHit.UnmarshalJSONObject(dec, key); v || e != nil {
		return e
	}
...
func (p *PSIBlast) ToCLI() *exec.Cmd {
	...
	p.QueryConfig.ToCLI(b)
	p.WithIPGListContainer.ToCLI(b)
	p.WithBestHit.ToCLI(b)
...
func (p *PSIBlast) Validate() bval.ValidationError {
	err := make(bval.ValidationBuilder)

	err.
		Validate(&p.QueryConfig, &p.WithIPGListContainer).
		IncompatibleWith(
			&p.QueryFile,
			&p.InMSAFile,
			&p.MSAMasterIDX,
		...
		IncompatibleWith(&p.DBFile, &p.SubjectFile, &p.SubjectLocation).
		U32GTEQ(&p.WordSize, 2).
		Validate(&p.Threshold, &p.CompBasedStats).
		IncompatibleWith(
			&p.SubjectFile,
			&p.GIList,
			&p.NegativeGIList,
			&p.SequenceIDList,
			&p.NegativeSequenceIDList,
			p.TaxIDs,
			p.NegativeTaxIDs,
			&p.TaxIDList,
			&p.NegativeTaxIDList,
	...
	if p.Format.Type == field.FormatTypeSequenceAlignmentMap {
		err.AppendError(p.Format.Flag(), "Format type 17 is not supported with psiblast")
	}

	err.IncompatibleWith(&p.CullingLimit, &p.BestHitOverhang, &p.BestHitScoreEdge).
		IncompatibleWith(
			&p.Remote,
			&p.GIList,
			&p.SequenceIDList,
		...
		IncompatibleWith(&p.InMSAFile, &p.PhiPatternFile, &p.InPSSMFile).
		IncompatibleWith(&p.MSAMasterIDX, &p.PhiPatternFile, &p.InPSSMFile).
		Validate(&p.MSAMasterIDX).
		Requires(&p.MSAMasterIDX, &p.InMSAFile).
		IncompatibleWith(&p.IgnoreMSAMaster, &p.MSAMasterIDX, &p.InPSSMFile, &p.PhiPatternFile)

	if len(err) == 0 {
		return nil
	}

	return bval.ValidationError(err)
}
